...
# Новый ретро-фон заката с землей
class RetroSunsetBackground:
    
    def __init__(self):
        # Сначала определяем параметры земли
        self.ground_y = 380  # Высота земли
        self.ground_color = (139, 115, 85)  # Цвет земли (коричневый)
        self.ground_detail_color = (121, 85, 61)  # Цвет деталей земли
    ...
    def create_static_background(self):
        ...
        # Отрисовка градиентного закатного неба (до земли)
        sky_height = self.ground_y
        for i in range(6):
            color = self.colors[i]
            rect_height = sky_height // 6
        ...
        # Статичный слой земли
        self.draw_static_ground()

    def draw_static_mountains(self):
        """Отрисовка статичных пиксельных гор"""
        # Создаем фиксированные горы с предопределенными позициями
        mountain_positions = [
            {'x': -50, 'y': 200, 'width': 400, 'height': 180, 'color': (80, 60, 40)},
            {'x': 300, 'y': 230, 'width': 500, 'height': 150, 'color': (70, 50, 30)},
            {'x': 700, 'y': 210, 'width': 450, 'height': 170, 'color': (90, 70, 50)},
            {'x': 1000, 'y': 220, 'width': 400, 'height': 160, 'color': (75, 55, 35)},
            {'x': 1400, 'y': 240, 'width': 350, 'height': 140, 'color': (85, 65, 45)},
        ]
    ...
    def draw_static_ground(self):
        """Отрисовка статичного слоя земли"""
        # Основной слой земли
        pygame.draw.rect(self.background_surface, self.ground_color, 
                        (0, self.ground_y, SCREEN_WIDTH, SCREEN_HEIGHT - self.ground_y))
        
        # Детали земли (неровности, камни)
        for _ in range(100):  # 100 деталей
            x = random.randint(0, SCREEN_WIDTH)
            y = random.randint(self.ground_y, SCREEN_HEIGHT - 10)
            size = random.randint(2, 8)
            shade = random.randint(-20, 20)
            stone_color = (
                max(0, min(255, self.ground_detail_color[0] + shade)),
                max(0, min(255, self.ground_detail_color[1] + shade)),
                max(0, min(255, self.ground_detail_color[2] + shade))
            )
            pygame.draw.circle(self.background_surface, stone_color, (x, y), size)
            
        # Линия горизонта
        pygame.draw.line(self.background_surface, (100, 80, 60), 
                        (0, self.ground_y), (SCREEN_WIDTH, self.ground_y), 2)
    ...
    def draw(self, screen):
        ...
        # Статический фон (небо, горы, звезды, земля)
...
def main():
    ...
    def draw_track():
        """Отрисовка беговой дорожки поверх земли"""
    ...
    while run:
        ...
        for obstacle in obstacles:
            ...
            if player.dino_rect.colliderect(obstacle.rect):
                ...
                death_count += 1
                menu(death_count, background, font)
                return  # ВАЖНО: выходим из цикла после проигрыша

        # Дорожка поверх земли
        draw_track()
...
def menu(death_count, background=None, font=None):
    global points, game_speed, obstacles, x_pos_bg, y_pos_bg
    run = True
    ...
    # Создаем шрифт для меню, если не передан
    if font is None:
    ...
    while run:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                return  # Выходим из функции menu
            if event.type == pygame.KEYDOWN:
                # Сбрасываем переменные перед новой игрой
                game_speed = 20
                obstacles = []
                x_pos_bg = 0
                y_pos_bg = 380
                run = False  # Выходим из цикла меню
                main()  # Запускаем новую игру
                return  # Выходим после завершения игры
        ...
        if death_count == 0:
            ...
            score_text = font.render("Your Score: " + str(points), True, (255, 204, 0))
